...
- **Шаблон ICP** — учебный пример в ICP stories, не verified architecture клиента.
...
**Источники:** voice notes / Plan v0.1 (~6 paying), about.thestage.ai/cases, GTM Framework (SaladCloud), industry press (Brilliant Labs + TheStage).  
...
|----------|------|---------|----------------------------------------|-----------------------------------|--------|
| **Recraft** | recraft.ai | Gen AI: image/vector, LoRA, 3M+ users | **☁️** NVIDIA **сейчас** · **📱** mobile on-device — **гипотеза** (phase 2) | Elastic (Flux), ANNA; GPU COGS | Paying; не на cases page |
| **Nebius** | nebius.com | AI cloud (H100/B200…) | **☁️** — inference **у конечных клиентов Nebius** на их GPU; TheStage = Platform на Nebius | CLI `--provider Nebius`, ~$1k/GPU/год license | Партнёр/infra |
| **Brilliant Labs** | brilliant.xyz | AI-очки **Halo** | **🔀** **📱** paired smartphone (TheStage + Neuphonic, press) + **🕶️** Alif NPU **on-glass** (не TheStage compile) · **☁️** — **TBD** | ANNA / deploy **phone path** | Press TechDay 2026 |
| **Phonic** | phonic.co | Speech-to-speech **voice agents**, enterprise | **☁️** serving **гипотеза** (sub-500ms cloud) · **📱** on-device capture для mobile sales — **уточнить** | Voice inference / latency | Paying; продукт TheStage **TBD** |
| **Usmile** | usmile.com | Smart-щётки, AI brushing | **🕶️** on-device в щётке/IoT **гипотеза** · **☁️**/**📱** — **неизвестно** | Имя в 6 paying only | **TBD**; не glasses/voice ICP |
...
|----------|------|---------|----------------------------------------|------------------|--------|
| **Praktika** | praktika.ai · App Store | AI language tutor, **~2M active** | **☁️** cloud **сейчас** (voice notes) → **📱** on-device **цель** (iOS first; Android **TBD**) · возможен **🔀** STT local + LLM cloud/local | Cloud → on-device migration; главный focus | **Факт** apps; split OS — **TBD** |

#### C. Публичные кейсы на about.thestage.ai/cases

| Кейс / клиент | Сайт | Домен | **Где AI** | Заявленный результат (TheStage) |
|---------------|------|-------|------------|----------------------------------|
| **SaladCloud** | salad.com | Distributed inference | **☁️** GPU network (Salad nodes) | **10k+ images per dollar** (ANNA) |
| **Wallarm** | wallarm.com | API security ML | **☁️** / **сервер CPU** (1 Gb/s per core — не phone) | **99%** malware detection |
| **Huawei P50/P60** | consumer.huawei.com | Camera HDR | **📱** **🕶️** Snapdragon 888 **NPU on-phone** (кастом compile) | 4K **&lt;1s**, **6×** speedup |
| **Nissan** | nissan-global.com | Automotive R&D | **☁️** / lab pipeline **гипотеза** (EEG model) | **+7%** accuracy |
...
| Имя | Сайт | Роль |
|-----|------|------|
| **Amazon AWS** | aws.amazon.com | GPU provider в Platform CLI |
| **Nebius** | nebius.com | GPU provider в CLI (см. также paying выше) |
...
|----------|:---:|:---:|:---:|----------|--------|
| **Brilliant Labs** | ◐ | ✓ TheStage press | ✓ Alif on-glass | TechDay 2026; Alif press | **Press**; iOS/Android split **TBD** |
...
|----------|------|-----------------|
| Halliday | halliday.ai | AI glasses, consumer |
| Gobi | (verify domain) | AI glasses startup tier |
| Even Realities | evenrealities.com | Lightweight AR + AI |
| Mentra | mentraglass.com | Open glasses + dev community |
...
**Как строить POV (любой аккаунт):** POV — how to build) — 3 stages (hypothesis → consensus → exec), MUD, Strategic Soundbite, value pyramid; методология ESC & Whyzer.

**Playbook S7 / public OEM:** POV — finance-first outbound) (VUZI, savings map, CEO/CFO vs ML).

**Worked illustration (VUZI):** Vuzix — POV and business case (illustration)) — полный GTM-флоу: MDP, POV, metrics, unit economics (`12k × $5`), business case, exec checklist (illustration only). **Словарь для нетехнических** — в начале того же файла.

**GTM strategy (finance-first motion):** GTM — finance-first motion (strategy & prep)) — landmark-клиенты (W3), метрики Tier 1–3, сигналы таргетинга, financial literacy checklist, operational backlog.
...
| **VP BD / OEM** | RFP-ready unit economics | Benchmark sheet ($/1k sessions) |
| **ML / platform** | tps, ttft, tiers; time-to-deploy | PoC benchmark |

**Правила:**

- Executive mail — **стратегия + P&L**; технический deep-dive отдельно (не tier S/M/L в первых 10 строках CEO).  
...
**Схема потоков:** executive (why + budget) → technical (how + proof) → sponsor sync → Phase 1. Детали: POV doc §12.md#12-executive-vs-ml-двухуровневая-продажа).
...
**Архитектурная оговорка (Vuzix):** M400 — embedded Speech SDK (local); Ultralite — AI on **Snapdragon AR1**; партнёры (e.g. Ramblr) — **cloud** contextual AI. Если **весь** новый AI только on-device без cloud → ценность TheStage = R&D cloud + будущие сервисы; приоритет аккаунта ↓.

**Discovery перед цифрой:** monthly cloud/GPU AI spend? кто платит inference в OEM pilot? самый дорогой workload (STT / LLM / vision)?

Полная карта: POV doc §11.md#11-vuzix-где-наибольшие-benefits--cost-savings).
...
### Публичный pricing (app.thestage.ai)
...
Ниже — **как это выглядит в жизни** команды клиента: боль → что пробовали → что даёт TheStage → что **остаётся** на стороне клиента. Развёрнутые «Reddit-истории» — в ICP use cases.
...
**Контекст (шаблон ICP + публичные данные по Brilliant):** малый hardware+ML; consumer AI glasses. **Не утверждаем**, что у Brilliant ровно «mic-only очки + весь inference на iPhone» — это было упрощение в ICP stories §2. Детальная карта: §3.8–3.9.

**Что известно публично про Brilliant Halo (май 2026):**

| Элемент | Источник | Вывод |
|---------|----------|--------|
| TheStage + Neuphonic + Brilliant, privacy-first on-device narrative | TechDay Mar 2026 | **Факт** партнёрства в прессе |
...
- **Benchmark sheet** p95 на iPhone 15/16 класса.
...
> **POV development:** всегда через 3-stage framework) (ESC) — consensus, champion ownership, POV ≠ business case.  
> **Enterprise wearables / public OEM:** finance-first POV) — CEO/CFO/BD + ML.
...
См. Diagram — ML workflow classic vs Stage AI.md (Mermaid: классика vs Stage, runtime wearable).
...
- **POV:** 3 stages + consensus + champion presents (framework) · ESC).
...
*Полный реестр (~45 вопросов):* StageAI Roadmap § Реестр
